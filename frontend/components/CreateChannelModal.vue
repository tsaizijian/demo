<template>
  <ClientOnly>
    <teleport to="body" v-if="isMounted && channelStore.showChannelCreator">
      <div
      ...
      >
        <div
          class="bg-white dark:bg-gray-900 rounded-xl shadow-2xl border border-gray-200"
          style="max-width: 32rem; width: 100%; max-height: 90vh; overflow-y: auto;"
          @click.stop
        >
          <UCard
            :ui="{
              ring: '',
              base: 'bg-transparent shadow-none border-none rounded-xl',
...
// 關閉 Modal
const handleClose = () => channelStore.closeChannelCreator();
const modalUI = {
  overlay: {
    base: "p-4",
    background: "bg-black/50 backdrop-blur-sm",
  },
  container: "relative w-full max-w-lg",
  base: "relative bg-white dark:bg-gray-900 rounded-xl shadow-2xl ring-1 ring-black/5 border border-gray-200",
};
...
// 監聽對話框關閉，重設表單
watch(
  () => channelStore.showChannelCreator,
  (isOpen) => {
    if (!isOpen) {
      // 重設表單
      form.name = "";
...
onMounted(() => {
  ...
  console.log(
    "CreateChannelModal mounted, showChannelCreator:",
    channelStore.showChannelCreator
  );
});
...
watch(
  () => channelStore.showChannelCreator,
  (newValue) => {
    console.log("showChannelCreator changed to:", newValue);
  }
);
</script>
