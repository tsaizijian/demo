<template>
  <UModal 
    v-model="channelStore.showChannelCreator"
    :ui="{ 
  ...
  >
    ...
    >
      <template #header>
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            <UIcon name="i-heroicons-plus-circle" class="w-5 h-5 text-blue-600" />
            <h3 id="create-channel-title" class="text-lg font-semibold text-gray-900">建立新頻道</h3>
          </div>
          <UButton
            @click="channelStore.closeChannelCreator()"
            variant="ghost"
      ...
      <div class="py-4">
        <p id="create-channel-description" class="text-sm text-gray-600 mb-4">
        ...
        <form @submit.prevent="handleCreateChannel" class="space-y-4">
          ...
          <div>
            ...
            <p class="mt-1 text-xs text-gray-500">頻道名稱將會自動轉為小寫，空格會被替換為連字符</p>
          </div>
          ...
          <div class="space-y-3">
            ...
            <div class="flex items-start gap-3">
              ...
              />
              <div class="flex-1">
                <label for="is-private" class="text-sm font-medium text-gray-700 cursor-pointer">
                  私人頻道
                </label>
                <p class="text-xs text-gray-500">只有受邀請的成員才能看到和加入此頻道</p>
              </div>
            </div>

            <!-- 最大成員數 -->
            <div>
              <label for="max-members" class="block text-sm font-medium text-gray-700 mb-1">
                最大成員數
              </label>
              <UInput
                id="max-members"
          ...
          <div v-if="form.name" class="bg-gray-50 rounded-lg p-3">
            <div class="text-xs font-medium text-gray-500 uppercase mb-2">預覽</div>
            <div class="flex items-center text-sm text-gray-700">
              <UIcon 
                :name="form.is_private ? 'i-heroicons-lock-closed' : 'i-heroicons-hashtag'" 
                class="w-4 h-4 mr-2"
              />
            ...
            <p v-if="form.description" class="text-xs text-gray-500 mt-1">{{ form.description }}</p>
          </div>
...
// 監聽對話框關閉，重設表單
watch(() => channelStore.showChannelCreator, (isOpen) => {
  if (!isOpen) {
    // 重設表單
    form.name = ''
    form.description = ''
